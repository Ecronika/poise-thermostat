...
coordinator and is live but opt-in — it fires only when both boiler on/off
actions are configured (``PoiseHubCoordinator._actuation``); with no actions
configured the hub stays shadow-only.
"""
...
# Coordinator-driven read-only entity: nothing to throttle per entity.
PARALLEL_UPDATES = 0
