...
## 12. Nachtrag: Stand v0.185.0 (2026-08-08) — Stufe A implementiert und im Feld

Die fünf Schritte aus §11 sind vollständig abgearbeitet; **Stufe A (§7) ist code-seitig komplett** — und in einem Punkt über den §7-Zuschnitt hinaus: Das dort vorgesehene `set_fan_mode`-Kommando ist per R2 zur echten kompressorfreien **Fan-first-Kühlstufe** geworden (Sequenz `normal → fan_only → fan_only+Stufe → cool`, echo-gegated, fail closed).

**§9-Bilanz:**

- **R2 umgesetzt** — [ADR-0068](../adr/ADR-0068-Luefter-als-erste-Kuehlstufe.md) (Implementiert, v1 Single-Aktor), gehärtet über vier Maintainer-Review-Runden: fail-closed-Stufenwahl (unbekannte Labels nie geraten, `auto` als Kommando ausgeschlossen — Firmware-Souveränität), mehrtickige echo-gegatete FSM (Dwell erst nach *beobachtetem* `fan_only`), Entry-Guard gegen laufenden/unbekannten Verdichter, Mode-Seam-Invariante bewusst geändert **mit Intent-Provenienz** (manuelle Kühlwünsche über beide Kanäle nie abgefangen), Guards als eigene Poise-Politik deklariert (35 °C-Grenze, Klemmen-Band 0,2→0,8 m/s). Feld-Nachtrag: Yield-/Release-**Restore** der Lüfterstufe (Vorher-Wert zuerst, `auto` nur als Fallback, nie über Fremdänderung oder unbestätigte Stufe).
- **N1 umgesetzt** — ADR-0055 bei ~55 %: zeitgewichteter PPD (`pmv_valid`-gegated, eigene Reife-Uhr), `meets_comfort_quality` mit Baseline-Nicht-Verschlechterung, Risiko-Tiers 1–3 mit fail-closed-Dispatcher (`flip_metric_ok`: unbekannt fällt zu).
- **R1 bewusst verneint** — Stufe-C-Entscheidung (2026-08-07): kein Befeuchter-Executor (EN-16798-Zurückhaltung bei Befeuchtung, REHVA-Energieargument, Hygiene-Unsichtbarkeit aus Sensor-Sicht); der §-Widerspruch ADR-0046-P5 vs. ADR-0048-§3 ist zugunsten von ADR-0048 aufgelöst (P5 = Entfeuchter-only), die sichere RH-Hülle bleibt für *fremde* Befeuchter publiziert, Revisit-Kriterien dokumentiert.

**§8-Fragen-Bilanz:** 1 (Regelgröße) **beantwortet** — beide Stufe-A-Größen wirken als operative Additive in `decide()` nach dem eco_widen-Muster (EN-Klemmen binden automatisch, Mode-Entscheidung und Sollwert sehen dieselbe Kante); PMV-Inversion mit ±0,3-Totband und ±1-K-Kappe. 2 (Feuchte) **entschieden** — Gesundheitsgröße, kein Komfort-Stellglied. 3 (M1) — N1 fertig, die Teilfrage Flip-Granularität ist damit **beantwortet** (pro Feature über Risiko-Tiers + eigene Tier-Latches, nicht global); **Feldkalibrierung läuft** (Gate vom Maintainer nachjustiert, unter Beobachtung). 4/5 (Stufe B) — [Aufwandsschätzung](../Konzepte/2026-08-07_Stufe-B-Aufwandsschaetzung_Multi-Aktor.md) liegt vor (P3 = XL; Kernrisiken `unique_id`-Kollision und climate-förmiger `ActuatorCommand`; **Nachtruhe-Konzept fehlt** — WP4-Lücke). 6 (Sensorik-Minimum) **beantwortet** — Degradationsleiter über `pmv_control_ready` und `presence_control_ready ∧ room_present` (Diagnose- und Regel-Gültigkeit getrennt, Ausfall fällt sicher zu). 7 (Zieldefinition/Kostenmodell) **offen**. 8 (Name) **entschieden** — „Aktive Behaglichkeit", Options-Toggle `active_comfort` (kein Preset).

**Umsetzungskern ([ADR-0069](../adr/ADR-0069-Aktive-Behaglichkeit-Stufe-A.md), Implementiert, alle 8 Schritte):** Readiness-Prädikate; persistierter Tier-2-Lebenszyklus `shadow→eligible→live→suspended` unter DiagnosticsRuntime (qualifizierte 24-h-Dwell — nie Wall-Clock —, Schmitt-Hysterese 1/2 pp, Baseline-Signatur inkl. Prädezessoren, Kaskadenregel, Serialisierung `fan_ce → pmv_offset` mit `impossible`-Escape); `set_fan_mode`-Executor-Primitive mit voller Own-Echo-Disziplin; Fan-first-FSM; Seam-Provenienz; **ein** Live-Pfad für Fan-first und ADR-0053-Umwälzung; Tier-2-Verdrahtung über Vortick-Latches. Card: Maßnahmen-Zeile („Behaglichkeit aktiv · …" / „reift"), „nicht validiert"-Hinweis (ADR-0054-V3-Vormerk eingelöst), Behaglichkeits-Anzeige als **100 − PPD** (Zufriedenheit statt roher Unzufriedenheitsquote), eigenes Level-Vokabular je Lampe.

**Erste Feld-Evidenz (zwei Zonen, zwei echte Befunde, beide behoben):**

- *Bad* (TRV, lüfterlos): Tier 3 korrekt inert, Byte-Identität bei Toggle-aus bestätigt, PPD-Reife wächst. Befund P1: Die Dwell-Serialisierung verklemmte an der strukturell unmöglichen `fan_ce`-Latch — behoben durch `impossible`-Retirement in `step_tier` (zieht auch eine bereits **persistierte** Fehl-Latch nach shadow zurück, stärker als reines Ready-Gating).
- *Büro* (fan-fähig): **erster realer Fan-first-Durchlauf spezifikationstreu inklusive Yield an den Verdichter.** Befund: Lüfterstufe blieb nach dem Yield auf `low` stehen — Restore-Entscheidung getroffen und implementiert (s. o.).

**Abgeleitete nächste Schritte (Stand 2026-08-08):**

1. **M1-Feldkalibrierung zu Ende beobachten** (läuft, Maintainer-seitig): erste Tier-2-Live-Übergänge abwarten — Bad `pmv_offset` (PPD-Reife + 24-h-Dwell, Größenordnung Tage), Büro `fan_ce` — und dabei Baseline-Stempelung, Schmitt-Verhalten und ggf. die erste Kaskade im Feld verifizieren; danach die konservativen Schwellen-Defaults gegen die Saisondaten bestätigen. Der ADR-0055-Restpunkt (Winter-Verdrahtung des Tier-1-Flips an PI/TPI) bleibt eigenes Heizsaison-Vorhaben.
2. **Nutzenbeleg führen:** PPD-Verlauf und Verdichterlaufzeit vor/nach den Tier-2-Freigaben (HDH-Schiene) — der im ADR-0068-Verifizierungsplan vorgesehene Feld-Nachweis, zugleich erster Stoff für §8.7 (Zieldefinition/Kostenmodell), falls Stufe B kommt.
3. **Ruhezeiten-Konzept designen** (WP4-Lücke): echtes `quiet_hours` statt des Komfortfenster-Proxys — Voraussetzung für ADR-0068-P7 und für saubere Nacht-Semantik erhöhter Stufen; kleiner Design-Schnitt (Sub-ADR oder ADR-0046-Nachtrag), unabhängig von Stufe B startbar.
4. **F2-Feedback-Felddaten** (ADR-0067 §5, weiter ausstehend): erste echte Button-Pressungen validieren die clo-Vorschlagsfamilie — und schärfen zugleich die dokumentierten Stufe-C-Revisit-Kriterien.
5. **Stufe B erst nach Stufe-A-Felderfahrung** (Empfehlung der Schätzung unverändert): dann P3 als eigenes Plan-Dokument im Phase-6b-Stil (Verträge zuerst, Byte-Gleichheits-Regression für Single-Nutzer), P4/P5 Entfeuchter-only, P6/P7 samt Guard-Test-Fortschreibung — der erste Fan-first-Durchlauf ist da, eine Saison ist es noch nicht.

**Damit ist der Bogen dieses Dokuments geschlossen — §1.3-Lückenbilanz:** Lücke 2 (Fan-Executor-Pfad) und 3 (PMV/SET-Eingriff als Sollwert-Additiv) sind gebaut, 5 (Feedback-Kanal) war es seit §11, 6 ist im Kern geschlossen (clo/met-Modell per N2, `pmv_valid`, Degradationsleiter definiert; CO₂-/velocity-Sensorik bewusst Tabellen-/Schätzpfad), 7 ist für den eigenen Aktor geschlossen (Fan-Kanal-Adoption `observe_fan_foreign`; Nebengeräte-Ownership bleibt Stufe-B-Materie). Offen bleiben die beiden bewusst vertagten: 1 (Multi-Aktor-Konfiguration — Stufe B, beziffert) und 4 (Befeuchtung — Stufe C, verneint mit Revisit-Kriterien). Was jetzt ansteht, ist keine Implementierung, sondern Saison: Kalibrierung, Nutzenbeleg, Felddaten.
