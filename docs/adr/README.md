...
| [0022](ADR-0022-Security-und-Supply-Chain.md) | Null schwere Deps, lokal, anonymisiert, redigiert | Implementiert |
...
**In Arbeit — prädiktiver Kern läuft durchgängig als Shadow** (berechnet & diagnostisch ausgegeben, schreibt aber den Aktor **nicht**, aktiv gated auf Kaltsaison-Validierung): MPC-Solver & Live-Verdrahtung (0001/0009/0033), TPI-Direktventil (0004/0036), PI-Sollwert-Kompensator (0037), prädiktive Verschattung (0043). Mehrzonen-Shedding/Vorlauf/Verdichter (0013) ist teils Shadow. Querschnittsthemen teilweise offen: Migration/Deprecation (0007/0018), Config-Auto-Discovery (0008), Performance-Budget (0020), i18n-Vollständigkeit (0021).

**Befund (Ehrlichkeit, Stand v0.163.0):** Die frühere Warnung zu **ADR-0044/0045** ist erledigt — beide sind seit v0.97.0 im Coordinator verdrahtet (Outcome-/Savings-Diagnose live) → „Implementiert". **ADR-0046 (Multi-Aktor)** ist über P0–P2 (Thermal-Shadow + Per-Device-Lifecycle) plus **live** Verdichterschutz (§8, v0.141) fortgeschritten → „In Arbeit (30 %)". Rein diagnostisch/Shadow (berechnet, **keine** Aktor-Writes): PMV/PPD (0054, `Live-D`), Regelgüte-Metrik (0055, `Live-D`), Referenzrahmen-Offset inkl. Laufzustands-Konditionierung (0056, `Shadow`). **Live aktuiert** (nicht mehr Shadow, daher `Live-A`): Feuchte-Dry (0050) und Hitzetag-Kühlband/Thermoschock (0051) schreiben Modus/Sollwert im Regelpfad, Aktor-Dynamik-Profile (0052) drosseln die Setpoint-/Modus-Nudges. **ADR-0019 (KNX-Expose)** weiter nicht begonnen (P2/optional) → „Vorgeschlagen".
