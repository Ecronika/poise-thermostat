...
> **Nachtrag §8 (P2-3, v0.166.0 — atomarer Modus-/Sollwert-Write):** Der Modus-Nudge (`set_hvac_mode`) und der Sollwert-Write waren getrennte Service-Calls; blockte der Verdichterschutz (min-off) den Nudge, lief der Sollwert-Write **trotzdem** und schob das Sollwert des *neuen* Regimes in ein Gerät, das noch im alten Modus stand (Kühl-Soll auf noch-Heizen). Fix (hybrid): (a) `actuator.service_call_for` trägt einen konditionierenden Modus (heat/cool/dry/heat_cool) **atomar** in denselben `set_temperature`-Call (HA-Core schaltet erst den Modus, dann den Sollwert); (b) solange der Guard den Wechsel hält (`_mode_nudge_blocked`), wird der Sollwert-Write des neuen Regimes **aufgeschoben** — das Gerät hält alten Modus **und** alten Sollwert, bis der Guard freigibt, dann flippt der atomare Write beides zusammen. Nicht-konditionierende Modi (off/idle/fan_only) tragen keinen Sollwert und bleiben dem dedizierten Modus-Pfad überlassen. Pure Abdeckung `tests/test_actuator.py` (Ride-along + Auslassung), Guard-Defer in `tests/integration/test_compressor_guard.py`.
