...
**Status:** In Arbeit (30 %) · **Wirkung:** teilw. · **Datum:** 2026-06-28 · **Bezug:** Poise v0.92.0; ADR-0005 (Pure-Core/Glue), ADR-0012 (Safety/Window/Heating-Failure), ADR-0013/0038 (Hub), ADR-0023 (Dual-Setpoint), ADR-0035 (Constraint-Solver, Präzedenz), ADR-0042 (Override).
...
> **Nachtrag §8 (P2-3, v0.166.0 — Guard-Defer des Setpoint-Writes):** Der Modus-Nudge (`set_hvac_mode`) und der Sollwert-Write sind getrennte Service-Calls; blockte der Verdichterschutz (min-off) den Nudge, lief der Sollwert-Write bislang **trotzdem** und schob den Sollwert des *neuen* Regimes in ein Gerät, das noch im alten Modus stand (Kühl-Soll auf noch-Heizen). **Ausgeliefert (v0.166.0):** der Setpoint-Write wird gegatet — solange der Guard den Wechsel hält (`_mode_nudge_blocked`), wird er **aufgeschoben** (`coordinator.py`, Setpoint-Write-`if`); das Gerät hält alten Modus **und** alten Sollwert bis zur Freigabe. **Zurückgestellt (NICHT implementiert):** der ursprünglich geplante *atomare* Mode+Setpoint-Write (`hvac_mode` mit im `set_temperature`-Call). Er wurde zurückgenommen, weil er über die Safe-State-/Frost-Rescue-Write-Pfade (die vor der Guard-Block-Berechnung laufen) den Verdichterschutz umgehen konnte — `actuator.service_call_for` bleibt **temperature-only**. Das Guard-Defer-Gate ist testabgesichert (`tests/integration/test_p2_gates_and_defer.py::test_guard_defers_setpoint_write_under_min_off`: min-off geseedet → `mode_nudge_blocked` gesetzt → kein `set_temperature`-Call). Bei Wiederaufnahme des atomaren Writes: nur opt-in via Command-Flag + Ride-along-/Defer-Tests.
...
**Umsetzung P2 (v0.92.0, Shadow):** `multi/lifecycle.py` ist pur implementiert — `DeviceLifecycle` (Wall-Clock-Zustand) + `LifecyclePolicy` (aus den `ZoneDevice`-Limits via `policy_for`, **Single-Source**), `observe()` (On/Off-Transition, Mode-Change, Start-Fenster), `min_off_`/`min_on_`/`mode_hold_remaining`, `starts_in_last_hour`, `to_runtime()` (→ Resolver-`DeviceRuntime`-Gate), `is_external_override()` (§9, ruht bis P3 schreibt) und `to_dict`/`from_dict` mit **konservativem Restore** (Future-Stempel → auf `now` geklemmt, Sperre bleibt eher aktiv als versehentlich abgelaufen). 19 Tabellentests. Der Coordinator faltet je Tick den realen Aktor-Run-State (`hvac_action`) **wall-clock** in die Lifecycle, leitet das `DeviceRuntime` ab und reicht es in den P1-Shadow (neue Diagnose `multi_min_off_remaining` / `multi_device_health`); die Lifecycle **persistiert in der Storage** (Save-Payload + Bootstrap-Restore) und überlebt so den Neustart — **keine** Aktor-Writes. **Grenze (ehrlich):** der Shadow wendet den konservativen Default-`min_off` (10 min) **einheitlich** an — für einen modulierenden Heiz-TRV ist das im Shadow eine wirkungslose Falsch-Sperre (kein Write); die **gerätegerechte** Policy (TRV `min_off=0`) greift mit der Per-Gerät-Konfiguration in **P3**.
...
**Umsetzungsstand:** **P0** (pures `multi/`-Paket: model/reason/discovery/schema/resolvers + Tests) **und P1** (Thermal-Shadow **live** im Coordinator — `multi/shadow.py:evaluate_thermal_shadow` baut den transienten `ZoneDevice` aus dem Aktor und exponiert die Diagnose-Attribute `multi_active_source` / `multi_reason` / `multi_severity` / `multi_blocked`; **keine** Aktor-Writes, Feuchte/Luft weiter no-op) sind umgesetzt; **P2** (Per-Device-Lifecycle, **v0.92.0**) ebenfalls — pure `multi/lifecycle.py` (Min-Cycle/Health/External-Override + **Wall-Clock-Persistenz**, konservativer Restore, 19 Tests) + Coordinator-Verdrahtung (observe → `DeviceRuntime` → Shadow, Diagnose `multi_min_off_remaining`/`multi_device_health`, **persistiert über Neustart**); einheitlicher Default-`min_off` im Shadow, gerätegerechte Policy in P3. Single-Active heute, der Seam ist live. Nächster Schritt **P3** (Thermal-Opt-in + Storage-Migration).
...
## Nachtrag (2026-07-03, v0.135.0): §8 Single-AC-Verdichterschutz (Shadow) — P3-lite

Der Roadmap-Punkt „Dry-Pfad-Kompressorschutz für den live schaltenden `dry`-Nudge" (bisher dämpfte nur die RH-Hysterese) ist als **Shadow-Vorstufe** verdrahtet — §8 gezielt auf den **Single-Aktor-Pfad** vorgezogen, ohne den vollen P3-Multi-Umbau.

**Befund (im Coordinator verifiziert):** Der Mode-Nudge (`set_hvac_mode`, der verdichter-taktende Write) war **ungated** — die ADR-0052-§4-`regulation_throttled`-Drossel deckt nur den Setpoint-Write und wird bei Modus-Wechsel bewusst umgangen. Allein die RH-Hysterese (`humidity_decide`) spaced das RH-getriebene dry-ein/aus; **cool↔dry-Flattern** an der Kühlkante (temperaturgetrieben, RH-hysterese-blind) blieb ungeschützt. Zweiter Befund: die P2-Lifecycle las `conditioning` aus `hvac_action` — eine AC, die **keine** `hvac_action` meldet (viele ESPHome/IR-Bridges, u. a. die Büro-AC), erschien damit **dauerhaft „aus"** → der Min-Off-Timer war auf genau diesem Gerät blind (`mode_hold` lief korrekt aus dem Modus-Echo).

**Umsetzung (Shadow):** (a) pures `compressor_running(hvac_action, intended_mode)` — Intent-Fallback (`cool/dry/heat`) wenn das Gerät keine `hvac_action` meldet (ADR-0024-cool-drive-Parität); der Coordinator füttert `conditioning` jetzt daraus → der Timer sieht den Verdichter. (b) pures `mode_nudge_block_reason(desired, current, min_off_remaining_s, mode_hold_remaining_s, is_safety)` — gibt einen Unterdrückungsgrund zurück, wenn ein Verdichter-**Start** (Eintritt cool/dry) bei `min_off_remaining>0` oder ein cool↔dry-**Moduswechsel** bei `mode_hold_remaining>0` anstünde; **reine Unterdrückung** (blockt nie einen Stop, erzwingt nie Weiterlauf → kein min_on-Überschwing-Risiko), **safety-exempt** (Fenster→off/Frost/Override nie geblockt). (c) Coordinator berechnet das Gate **vor** dem `observe()` (pre-observe = Live-Gate-Semantik) und exponiert `compressor_gate_would_block` + `compressor_mode_hold_remaining` als Diagnose/`_ATTRS`; `multi_min_off_remaining` wird durch den conditioning-Fix real. **Keine Unterdrückung** — der Nudge feuert weiter. Defaults aus §8 (`min_off` 600 s, `mode_hold` 300 s), wall-clock-persistent (P2), clock-skew-fest.

9 pure Tests (`test_multi_lifecycle`, 28 gesamt grün; ruff/format/mypy --strict clean via /tmp). **Live-Flip** (echte Unterdrückung) folgt nach Beobachtung am Büro-AC, dann gegated auf `can_cool` (nur echte Verdichter-Geräte; heiz-only TRV bleibt außen vor — behebt die P2-Grenze „einheitlicher min_off auch für TRV"). Damit rückt §8 für den Single-AC von „einheitlich-blind-Shadow" auf „gerätegerecht-sichtbar-Shadow".

## Nachtrag (2026-07-04, v0.140.0 → v0.145.0): §8 Verdichterschutz **LIVE** (Single-AC)

**Status: Implementiert (LIVE) für den Single-Aktor-Pfad.** Der Shadow-Vorbau (v0.135.0, oben) ist scharfgeschaltet:

- **v0.140.0 — Live-Flip.** `mode_nudge_block_reason` unterdrückt jetzt real den verdichter-taktenden `set_hvac_mode`-Nudge im Write-Pfad (`if _mode_nudge and _guard_block: _mode_nudge = False`), statt nur zu diagnostizieren. **Capability-gegatet** über `resolve_guard_policy(enabled, can_condition = can_cool ∨ „dry"∈modes, …)` → `None`-Policy (gar kein Gate) für heiz-only TRV/Hydronik (behebt die P2-Grenze „einheitlicher min_off auch für TRV"). **Kill-Switch** `compressor_guard = auto|off`; **Timer 300 s/300 s** — bewusst nicht die 600 s aus dem §8-Entwurf: die Geräte-Firmware erzwingt real bereits ~180 s Min-Off, 300 s ist der konservative koordinatorseitige Boden (per Zone überschreibbar). **Kein Starts-Cap** für den Single-AC (min_off begrenzt die Startrate strukturell; `max_starts_per_h` bleibt Hub-Gruppen-Territorium). **is_safety** unverändert (Fenster→off/Frost/Override/Frozen nie geblockt). Der geblockte Nudge ist reine **Verzögerung**, nie Safety: der Komfortwunsch steht und feuert nach Timer-Ablauf; `mode_nudge_blocked` liest als **Intent** (ein geblockter dry-Eintritt hält `dry_active` gelatcht).
- **v0.141.0 — Config-UI.** `compressor_guard` (Select auto/off) + `compressor_min_off_s`/`compressor_mode_hold_s` (Number, leer = Profil-Default) im Options-Flow + i18n (EN/DE).
- **v0.142.0 — LTS-Diagnose.** `compressor_guard_blocked` als Measurement-Sensor (Feld-Kalibrierbasis über den Recorder, nicht nur ~10-Tage-Attribut-Historie).
- **v0.145.0 — Fast-Follow abgeschlossen.** Card-Chip `Verdichterschutz: min-off Ns` (aus `mode_nudge_blocked`, i18n EN/DE) macht die Unterdrückung am Dashboard sichtbar; HA-Runtime-Integrationstest (`tests/integration/test_compressor_guard.py`) beweist die Live-Verdrahtung: das Gate hält den Kühl-Start bei aktivem Min-Off zurück (`compressor_gate_would_block` = „min-off …s", kein `set_hvac_mode(cool)`), der Kill-Switch entfernt das Gate → derselbe Start feuert. Pure Logik: 6 `test_multi_lifecycle`-Tests (`resolve_guard_policy`/`guard_block_reason`, inkl. Regression block→abgelaufen→feuert).

**Ehrliche Grenze:** nur Single-Aktor-Pfad; der volle Multi-Aktor-P3-Umbau (per-Device-Policies statt einheitlichem Default) bleibt offen. Die Coordinator-Verdrahtung ist CI-/live-gegatet (der Integrationstest läuft nur in CI; Sandbox-HA 2023.7 überspringt das Verzeichnis).

## Nachtrag §9 (2026-07-21, Phase-9-Refactoring-Migration): Warum `_lifecycle.observe()` spät faltet (K2b)

**Invariante (test-gepinnt):** Der Lifecycle-Fold `_lifecycle.observe()` läuft **spät** — im `finalize_tick`, **nach** der Verdichterschutz-Diagnose (`guard_block_reason`), nicht davor. Das ist kein zufälliger Anordnungsartefakt, sondern absichtlich: der späte Fold verrechnet das Ergebnis **dieses** Ticks für das Urteil des **nächsten** Ticks. Ein Hochziehen vor die Guard-Berechnung sieht wie eine reine Umsortierung aus, ist aber aus zwei unabhängigen, vom Integrationssuite bewiesenen Gründen falsch:

1. **Intent-Fallback-Zirkularität.** `compressor_running(hvac_action, intended_mode)` fällt auf Poises **eigenen Intent** zurück, wenn das Gerät keine `hvac_action` meldet (viele ESPHome-/IR-Bridges — ADR-0024-cool-drive-Parität). Vor dem Guard gefaltet, markiert der Start-Intent („wir wollen kühlen") das Gerät bereits als laufend → das `min_off`-Fenster verdampft → der Guard kann einen Start nie mehr bremsen, weil er gegen seinen **eigenen Intent** urteilt. Zirkulär und das exakte Gegenteil des Guard-Zwecks.
2. **`mode_changed_wall`-First-Tick-Falle.** `observe()` stempelt `mode_changed_wall = now`, sobald der beobachtete Modus vom gespeicherten abweicht — und auf dem **ersten Tick nach jedem Restart** ist der gespeicherte Modus `None`. Der Guard würde dann den Nudge desselben Ticks gegen einen Mode-Hold blocken, den er sich gerade **selbst** armiert hat: kein dry-/cool-/heat-Eintritt auf dem ersten Tick (pinnt `test_dry_actuation.py::test_dry_nudge_when_humid_and_idle`).

**Deckungsgleiche Fälle.** Dieselbe „Shadow rechnet immer, Fold spät"-Logik trägt (a) den **F1-Fix** in `control/tick_pipeline.py` (~Z. 964): die Mode-/Guard-Policy-Auflösung wurde aus dem `if self._enabled:`-Block herausgezogen, weil der unbedingte Shadow-Block (`multi_lifecycle`-Observation, ADR-0026 „Schatten-Schätzer rechnen immer") `final_mode`/`_guard_pol` auf **jedem** Tick liest — eine deaktivierte Zone warf dort sonst jeden Tick einen (geschluckten) `UnboundLocalError` und fror die Wall-Clock-Lifecycle still ein; und (b) die **diagnostische Spiegelung** desselben Gates, das der §8-Nachtrag im Write-Pfad aufschiebt (`_comp_block = _guard_block`, gegen die pre-observe-Lifecycle ausgewertet).

**Korrekter K2b (Redesign, nicht Umsortierung).** Ein sauberer K2b müsste nur den **geräteseitig gemeldeten** Run-State falten (nie den Intent-Fallback) und dürfte den Mode-Hold **nicht** auf einer Erstbeobachtung armieren — eine designte Änderung an `multi/lifecycle.py` mit eigenen puren Tests, nicht das Verschieben eines Statements.
