...
**Status:** Implementiert (v1, v0.163.0) · **Wirkung:** Live-A · **Datum:** 2026-07-11 · **Bezug:** ADR-0042 (Override/Preset-Modell, implementiert — wird *erweitert*, nicht abgelöst), ADR-0025 (Zeitplan/Optimal-Start), ADR-0058 (Presence), ADR-0035 (Constraint-Solver), ADR-0016/0040/0057 (Card-Vertrag), ADR-0008 (Config), ADR-0012 (Repair-Issues), ADR-0019 (KNX-Expose, künftig) · **Grundlage:** `docs/Meinungsbild_Manueller-Eingriff-Sollwert-und-Modus.md` (verifizierte Wettbewerbs-/Community-/Norm-Recherche, 2026-07-11)

> **Umsetzungsstand (v0.163.0):** v1 **vollständig** — HoldPolicy (schedule/timer/permanent), Boost-Timer, Presence-Ende, angekündigte Ablaufzeit, Event `poise_override_ended` + Service `poise.resume_schedule`, Feedback-Attribute + Card-Hold-Pill, Config-Sektion + Migration, L1-Erfassung **und §3 Preheat-Glättung** (v0.163.0: ein `schedule`-Hold endet an der steigenden Flanke des Optimal-Start-Preheat, wenn das Komfortziel über dem gehaltenen Wert liegt — pure `hold_ends_at_preheat`, im Tick verdrahtet). Der **Vorschlags-Teil (L2)** und der optionale **Modus-Saison-Hinweis** sind nach **[ADR-0060](ADR-0060-Override-Vorschlags-Lernen.md)** ausgegliedert (Status Vorgeschlagen); die dafür nötige L1-Statistik liegt bereits vor. Damit ist der Override-**Lebenszyklus** abgeschlossen.
>
> **Nachtrag v0.162.2/.3 (Live-Befunde):** (a) die Karten-Sollwert-Anzeige (Dial-Zahl, Graph, +/−-Basis) las die Bandkante `heat_sp` statt des Schreibziels `temperature` → auf `temperature` korrigiert. (b) **Regel-Gap:** ein aktiver manueller Override setzte nur den geschriebenen Sollwert, nicht den Modus — `decide_mode` bekam weiter das Norm-Band, sodass ein Soll < Ist auf einem kühlfähigen Gerät nicht kühlte (blieb im geparkten Heiz-/Idle-Modus). Fix: ein aktiver Override treibt jetzt den Modus wie ein Single-Setpoint-Thermostat (pure `override_mode` = Band ±0,5 K um den Override → `decide_mode`; im Coordinator als `base_mode` verdrahtet, nur ohne Fenster/Frozen). Geschriebenes Ziel + Frost-/Schimmel-Floor + Außen-Lockout + Feuchte-Gating bleiben erhalten.
...
### Nachtrag §8 (P1-4a, v0.168.0 — Adoption geräteseitiger Sollwertänderungen)

Ändert der Nutzer den Sollwert **am Gerät selbst** (TRV-Rad, Hersteller-App), meldete der Aktor bislang einen Wert, den Poise im nächsten Tick (≤ 60 s) überschrieb — der größte verbleibende Anwenderschmerz aus dem v0.163-Review (P1-4a). **Ausgeliefert (Live):** die Änderung wird als **manueller Hold mit der Zonen-`override_policy`** übernommen (`set_override`, norm-geklemmt), konsistent zu einem über die Poise-Entity gesetzten Hold, statt überschrieben zu werden. Pure `control/override.detect_external_setpoint` trennt eine echte Änderung vom **Echo des eigenen Writes** (Vergleich gegen den zuletzt gesnappten `_last_written_sp`) über zwei Guards: Baseline (noch kein eigener Write → keine Adoption) und Echo-Fenster (`SETPOINT_ADOPT_ECHO_WINDOW_S = 120 s` nach eigenem Write, gegen Zigbee-/Poll-Lag). Der Coordinator setzt bei Treffer den Override und **überspringt den Overwrite dieses Ticks** (der neue Ziel-Wert des nächsten Ticks spiegelt bereits den Hold). Gegated auf `device_schedule_active` (läuft ein Geräte-Zeitplan, bewegt dieser — nicht der Nutzer — den Sollwert) und die Zonen-Option **`adopt_external_setpoint`** (Default an, Opt-out je Zone, Sektion „Manuelle Eingriffe"). Tests: pure `tests/test_adopt.py`, Integration `tests/integration/test_setpoint_adoption.py` (Adoption→Hold + kein Overwrite; Echo-Fenster→kein Hold; Opt-out→kein Hold). Offen für eine Folgeiteration: der atomare Mode+Setpoint-Write nur opt-in (ADR-0046 §8), und die Anzeige-Semantik (`hvac_action`/Card-Referenzrahmen bei aktivem Override).

### Nachtrag §9 (2026-07-21, Phase-9-Refactoring-Migration): Adoption-Baseline **nicht** hold-gegatet beim Restore (B5)

**Invariante (test-gepinnt):** Die vier Adoption-Baseline-Werte (`last_written_sp`, `prev_device_sp`, `last_commanded_hvac`, `prev_device_mode`) werden beim Bootstrap **bedingungslos** wiederhergestellt — **nicht** hold-gegatet wie die Hold-Lifecycle-Felder daneben. Begründung: die Baseline beschreibt den **Aktor** (den zuletzt kommandierten und den zuvor beobachteten Gerätezustand), nicht den Hold. Ohne sie klassifiziert die Fremdänderungs-Erkennung (§8) die erste geräteseitige Intervention direkt nach einem Neustart als `no_baseline` (kein eigener Write als Vergleichsanker) und der nächste Poise-Write **revertiert sie still** — genau der Anwenderschmerz, den §8 beheben sollte. Das Restaurieren von `prev_device_*` **neben** dem Kommando ist zusätzlich das, was ein versetztes Gerät auf `stable_offset`/`stable_prev` hält, statt es bei jedem Neustart phantom-zu-adoptieren (F1/F2).

**Abgrenzung zu den transienten Feldern.** Die **monotonen** Echo-Stempel (`last_sp_write_ts`, `last_hvac_cmd_ts`) und der Attempt-State-Context-Ring sind prozess-lokal und liegen bewusst **nicht** im Schema: semantisch kann kein Echo einen Neustart überdauern, deshalb stempelt der Restore-Pfad die Echo-Fenster als **abgelaufen** (`now - ts >= echo_window`), statt sie zu resurrektieren — nur dort, wo tatsächlich eine Baseline existiert, sodass `no_baseline` sonst weiter gewinnt. Das ist zugleich der ehrliche Eingang für die ADR-0052-§4-Nudge-Drossel (es hat kürzlich kein Write stattgefunden). Diese Stale-Stempelung ist ein **Domänen-Hook** im Coordinator/`ZoneRuntime`, absichtlich **nicht** im Codec.

**Pinning:** `tests/integration/test_override_bootstrap_fixes.py` (Adoption unmittelbar nach Restart statt no_baseline-Revert), `tests/test_phase3_codec.py` (Roundtrip der persistierten vs. nicht-persistierten Felder), `tests/test_phase7_tracker.py` (Own-Write-Echo-Split gegen die Baseline). **Code-Ort:** `runtime/zone_runtime.py` (Restore-Zuweisung + Stale-Stempelung), `runtime/state.py::ExternalOverrideRuntime.PERSISTED_FIELDS`, `persistence/codec.py` (Baseline-Sektion, monotone Stempel nie im Schema).
...
Erweitert ADR-0042 (dessen „OFFEN"-Punkte hiermit entschieden sind) und ADR-0016/0040/0057 (Card-Vertrag um Override-Attribute). Konsumiert ADR-0025 (Schaltpunkte, Optimal-Start als Rückkehr-Glättung) und ADR-0058 (Haus-Gate als Ende-Trigger). Liefert die Ablauf-/Quelle-Semantik für ADR-0019 (KNX: DPT 20.102 Auto=0 als Resume, begrenzte Sollwertverschiebung, Ablaufzeit analog `SetpointHoldExpiryTimestamp`). Folge-Entscheidungen: **[ADR-0060](ADR-0060-Override-Vorschlags-Lernen.md)** (L2-Vorschlags-Lernen + Schwellen-Feld-Tuning + optionaler Modus-Saison-Hinweis, Status Vorgeschlagen), Frost-/Aus-Preset (ADR-0042 §4, weiterhin offen).
