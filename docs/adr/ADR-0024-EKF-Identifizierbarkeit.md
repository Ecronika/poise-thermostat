...
## Nachtrag (2026-07-05, v0.149.0): Settle-basierte τ-Konfidenz (Shadow)

Ergänzt das Zähler-Gate `identified` um ein **Konvergenz-Signal**: pures `estimation/tau_settle.py` (`TauSettle` — EWMA von α über **lern-aktive** Zeit: `mean`/`var`/`minutes` + `settled`; `rel_spread = std/mean`; `update_settle` hält bei idle/`None`/α≤0; `settle_confidence` → [0,1]) beantwortet „ist α wirklich stehengeblieben?" statt nur „genug Ticks gesehen?". Der zuvor angedachte **Kovarianzweg ist verworfen** (Closed-Loop v0.144: die α-Random-Walk-Prozessvarianz `_Q[_A]` floort `p[α][α]`, es schrumpft nie → als Konfidenz untauglich). Gefüttert nur auf lern-aktiven Ticks (dieselbe Anregung `u_h>0 ∨ u_c>0`, wo α sich bewegen kann); Idle-Ticks ließen ein eingefrorenes α trivial „settled" aussehen und sind ausgeschlossen — die Warm-up-Zeit (Default 60 min) zählt damit echte Lernzeit. Coordinator-**Shadow** (Persistenz `tau_settle`, Diagnose/`_ATTRS` `tau_confidence`/`tau_settled`/`tau_settle_minutes`) — **keine Writes**. 7 Unit-Tests (steady α → settled + Konfidenz 1.0; sign-flipping → nicht settled + 0; Idle verschmutzt das Fenster nicht; konservativer Restore); /tmp ruff/format/mypy --strict/pytest clean. **Offen (Folge, Phase 4):** die Konfidenz als **Klemme** auf den Optimal-Start-Preheat-Vorlauf (nicht binäres Gate), hinter dem ADR-0055-Flip-Gate; `rel_gate = 0.04` ist konservativer Default und wird offline gegen echte α-Trajektorien aus den Feld-Traces (ADR-0011) nachkalibriert.
