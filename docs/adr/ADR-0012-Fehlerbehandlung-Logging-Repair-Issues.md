...
## Nachtrag (2026-07-21, Phase-9-Refactoring-Migration): Stage-Checkpoint-Emissionsordnung, F12-Tick-Raise-Sichtbarkeit, V4-Disabled-Zone-Ausnahme

**(a) Repair-Issue-Emission an Stage-Checkpoints (finding 13).** Repair-Issues werden **während** des Ticks emittiert — frühere Issues bleiben gesetzt/gelöscht, auch wenn ein späterer Schritt fehlschlägt. Nach dem Phase-0-Refactoring geben die Stages ihre Issue-Übergänge als typisierte `HealthUpdate`-Objekte **zurück** (Feld-für-Feld-Spiegel des `_issue()`-Primitivs, inkl. der Übersetzungs-`placeholders` — sie wegzulassen verlöre beobachtbare Diagnose), statt die Issue-Registry selbst anzufassen; der Coordinator emittiert sie an **Stage-Checkpoints**, deren Positionen die heutigen Emissionspunkte **relativ zu den Awaits** erhalten. Bricht eine Stage ab, transportiert `TickStageError` die **noch nicht emittierten** `pending_health_updates`; der Coordinator emittiert sie und re-raist danach `cause`. **Positionsbeweis:** das Exception-Unwinding ist synchron (auch durch die async-Frames), sodass diese Emission im **selben Event-Loop-Turn** wie der Fehler läuft — kein Suspension-Point zwischen dem historischen „schon vor dem Fehler emittiert"-Zustand und diesem Checkpoint, kein fälliges Update geht verloren. Der `raise` steht **außerhalb** des `except`, damit kein impliziter Exception-Kontext an `cause` gekettet wird.

**(b) F12 — unsichtbarer Tick-Raise.** Ein aus `_run_once` geworfener Tick war früher jenseits von `DataUpdateCoordinator`s generischem „update failed"-Log/Entity-Unavailability **unsichtbar** — kein Poise-eigenes Signal, das einen einmaligen Blip von einer dauerhaft fehlschlagenden Zone unterscheidet. `_async_update_data` zählt jetzt aufeinanderfolgende Fehler (wie `_note_save_result` es für den Store tut) und emittiert nach 3 in Folge das Repair-Issue `tick_failing`; die Exception selbst wird **immer unverändert re-raist**, sodass `DataUpdateCoordinator`s eigene Fehlerbehandlung unangetastet bleibt.

**(c) V4 — Disabled-Zone-Safety-Ausnahme.** Eine **deaktivierte** Zone erhält weiterhin bedingungslosen Frost-/Schimmelschutz (README-Versprechen), aber **rescue-only** — ein vernünftiger manueller Sollwert über dem Boden wird nie bekämpft; ein cool-only-Gerät hat keine Frost-Pflicht (`frost_rescue_target → None`). Ein per Fernbedienung **`off`-gehaltenes** Gerät (Poise noch enabled, K2) wird wie eine deaktivierte Zone behandelt, aber — anders als eine echt deaktivierte Zone — **nur** dann gerettet, wenn der **Raum** tatsächlich am Frost-/Schimmelboden ist (sonst würde das absichtlich ausgeschaltete Gerät neu gestartet). Der Rescue-Write ist zusätzlich auf `_actuator_online` gegatet (F2-Follow-up: sonst würde ein toter Aktor jeden Tick ein `set_temperature` ins Leere geschickt bekommen).

**Pinning:** `tests/integration/test_phase6_health_checkpoints.py` (Checkpoint-Ordnung inkl. Abort-Transport), `tests/integration/test_phase0_health_emission.py` (F12-Failure-Counting + beide Emissions-Richtungen), `tests/integration/test_frost_rescue_disabled.py` (V4-Disabled-Zone-Rescue). **Code-Ort:** `coordinator.py` (`_async_update_data` F12, Stage-Checkpoints, Abort-Checkpoint, V4-Rescue-Block), `runtime/tick_result.py::HealthUpdate`/`TickStageError`.

**Nachtrag v0.182.0 (ADR-0066 B.5 + Redaction):** Die Lüftungs-Emission (Bus-Event + opt-in `persistent_notification`) läuft in einer **eigenen** Swallow-Grenze im Orchestrator (`_announce_vent_advice`) — ein Zustellfehler bricht nie den Tick und markiert nicht die Shadow-Grenze; die Notification ist bewusst KEIN Repair-Issue (transienter Zustandshinweis, kein Defekt — die P2-8-Linie „Repair statt Notification" gilt für Fehlerzustände, nicht für Ratschläge). `REDACT_KEYS` + `outdoor_humidity_sensor` (gleiche Id-Hygiene wie alle Sensor-Felder).
