...
- **Berührt ADR-0066** (Feuchte-Achse): `comfort/mold.py::max_safe_rh` bleibt bestehen und bekommt seine Grenze künftig als `MouldRisk.critical_rh / 100.0` statt als feste 0,80 — beide Achsen rechnen damit gegen dieselbe temperaturabhängige Linie. `surface_rh_mean` und der Lüftungsrat bleiben unverändert.
  **Präzisierung (2026-09-13, ADR-0066 N4.4):** „beide Achsen" meint die **Decke** `rh_max_safe` — nicht jede Regel, die über Schimmel spricht. Der Öffnen-Rat `mold_risk` (ADR-0066 Regel 1) prüft weiterhin gegen das **feste** 80-%-Limit, und das ist Absicht: ein niedriges `rh_max_safe` bedeutet eine **kalte** Oberfläche, und die spricht fürs Schließen, nicht fürs Öffnen. Regel 1 fragt absolut („sind die Oberflächen nass"), `mold_guard` relativ („mehr, als diese Wand verträgt"). Der Versuch, beide auf die Decke zu ziehen, wurde gebaut und am N2-Regressionsfall widerlegt; die Begründung steht in ADR-0066 N4.4.
